...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_type):
        ...
        self.examples = []
        source_path = file_path + ".buggy-fixed.buggy"
        target_path = file_path + ".buggy-fixed.fixed"
...
def main():
    ...
    ### specify device ### 
    # comment out the below two lines if needs to run on multiple gpus
    # args.n_gpu = 1
    # args.device = "cuda:0"
    ### ###
    ...
    #tokenizer =T5TokenizerFast(vocab_file='huggingface_model/data/android_with/vocab.json', tokenizer_file='huggingface_model/data/android_with/tokenizer.json')
